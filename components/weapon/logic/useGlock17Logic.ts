...
export const useGlock17Logic = (
    ...
    const sprintBlockTime = useRef(0); // Timestamp until which sprint pose is blocked (Gun stays in Hip/Ready)
    ...
    // GLOCK 17 SPECIFIC: HIGH PORT SPRINT (Muzzle Up)
    const sprintPose = {
        pos: new THREE.Vector3(0.0, 0.35, 0.3),
        rot: new THREE.Euler(0.0, 0.0, -1.57) // -90 deg Z to point up
    };
    ...
    const executeFire = () => {
        ...
        const heatAdd = isAiming.current ? 0.8 : 0.2; 
        recoilHeat.current = Math.min(recoilHeat.current + heatAdd, 1.0); 

        const baseSpreadRadius = isAiming.current ? 0.0 : 0.06;
        const heatPenaltyFactor = isAiming.current ? 0.10 : 0.15;
    ...
    const triggerAttack = () => {
        ...
        // We are in sprint pose IF: Player is physically sprinting AND we are not already "Combat Ready" (sprintBlockTime expired)
        const isInSprintPose = isSprintingEffective.current && (now > sprintBlockTime.current);

        if (isInSprintPose) {
            // CASE 1: GUN IS UP/DOWN (Running)
            // We must transition to Hip first.
            
            // 1. Force visual transition to Hip/Idle immediately
            // We set block time to (Now + Delay + Hold Time)
            // 0.5s to lower gun, then 1.0s hold.
            sprintBlockTime.current = now + 500 + 1000;
            
            // 2. Queue the shot to happen after the transition (0.5s)
            queuedShotTimer.current = 0.5; 
            
            return;
        } else {
            // CASE 2: GUN IS ALREADY READY (Walking, Standing, or already shooting while running)
            // Fire immediately.
            executeFire();
            
            // Extend the "Ready State" timer by 1.0s from NOW
            // This keeps the gun in Hip pose for 1s after the LAST shot
    ...
    useFrame((state, deltaRaw) => {
        ...
        // Handle Queued Shot (Sprint-to-Fire delay)
        if (queuedShotTimer.current > 0) {
            // While queueing, keep refreshing sprint block so gun stays down
            // We want it to stay down for at least 1s after the shot fires
            sprintBlockTime.current = Date.now() + 1000;
        ...
        // Keep blocking sprint if aiming
        if (isAiming.current) {
        ...
        aimingProgress.current = THREE.MathUtils.lerp(aimingProgress.current, targetAim, delta * 18); // Glock snaps faster
        ...
        recoilHeat.current = Math.max(0, recoilHeat.current - (delta * 3.0)); // Glock cools faster
        ...
        if (isAttacking.current) {
            ...
            const duration = 200; // Faster cycle than CZ
            const progress = elapsed / duration;
            
            if (progress >= 1) {
                isAttacking.current = false;
            } else {
                const recoilMod = 1.0 - (aimingProgress.current * 0.8); // High stability when aiming

                if (progress < 0.2) {
                    const kickX = (Math.random() - 0.5) * 0.05 * recoilMod; 
                    const kickY = (0.02 + Math.random() * 0.02) * recoilMod;
                    const kickZ = -0.30 * recoilMod; // Less kickback
                    
                    targetAttackRot.set(kickX, kickY, kickZ); 
                    targetAttackPos.set(0, 0.02 * recoilMod, 0.25 * recoilMod); 
                    attackLerpSpeed = 70 * delta;
                } else {
                    targetAttackRot.set(0, 0, 0);
                    targetAttackPos.set(0, 0, 0);
                    attackLerpSpeed = 15 * delta;
                }
    ...
    return {
        ...
        smoothAttackRot,
        sprintPose,
        sprintBlockTime // Export this so the main controller knows when to suppress sprint pose
    };
};